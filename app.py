...
def format_kr_official_disclosures(stock_code):
    ...
    return "\n".join(
        f"📌 공시<br><span class=\"news-meta\">DART · {item['date']}</span>"
...
def format_us_sec_filing(filing):
    ...
    if form == "4":
        ...
        return f"👤 내부자 거래<br><span class=\"news-meta\">SEC · {display_date}</span>"
    ...
    if form == "8-K":
        ...
        return f"📌 공시<br><span class=\"news-meta\">SEC · {display_date}</span>"
...
def analyze():
    ...
    try:
        ...
        # 뉴스 목록은 화면 표시 형식만 변경한다.
        # 기존 뉴스 수집/선별/AI 후보 데이터는 그대로 유지한다.
        news_candidates = get_news_ai_candidates(raw_name, limit=3)
        news_display_items = []
        for item in news_candidates:
            ...
            try:
                ...
                if parsed.tzinfo is not None:
                    parsed = parsed.astimezone(datetime.timezone(datetime.timedelta(hours=9)))
                display_time = parsed.strftime("%H:%M")
            except Exception:
                pass
            news_display_items.append(
                f"📰 뉴스<br><span class=\"news-meta\">{source} · {display_time}</span>"
            )

        news_lines = (
            "\n".join(news_display_items)
            if news_display_items
            else "📰 뉴스<br><span class=\"news-meta\">관련 뉴스 확인 대기</span>"
        )
