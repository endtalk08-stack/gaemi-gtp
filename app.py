...
def fetch_realtime_news(stock_name):
    try:
        ...
        with urllib.request.urlopen(req, timeout=3) as resp:
            ...
            headlines = []
            for item in items[:2]: # 너무 길어지지 않게 2개만 추출
...
def get_live_calendar_data(stock_name, ticker_symbol):
                ...
                with urllib.request.urlopen(req, timeout=3) as resp:
                    ...
                    if earnings_list:
                        ...
                        if act is not None and est is not None:
                            ...
                            earnings_msg = f"실적 소식으로는 방금 {kr_label} 실적이 발표됐는데 {status} 실제 EPS가 ${act:.2f}로 찍혔으니 참고해 둬."
                        elif est is not None:
                            # YYYY-MM-DD 형식을 O월 O일로 변환
                            try:
    ...
    if earnings_msg:
        return f"{macro_msg} {earnings_msg}"
    else:
        return f"{macro_msg} 지금은 개별 종목 수급 장세인 만큼 세력 평단가와 수급 턴어라운드 타이밍에 집중하자."
...
def analyze():
    ...
    try:
        ...
        if news_list:
            if len(news_list) > 1:
                news_lines = f"오늘 장중에 보니까 '{news_list[0]}' 이런 이슈가 가장 컸고, 이어서 '{news_list[1]}' 소식도 돌면서 시장이 반응하더라구."
            else:
                news_lines = f"오늘 장중에 보니까 '{news_list[0]}' 이슈가 돌면서 시장이 크게 반응하더라구."
        else:
            news_lines = f"지금 딱히 두드러지는 뉴스 기사보다는 세력들 수급 눈치싸움으로 주가가 움직이는 거 같아."
        ...
        if foreign_5d is not None and inst_5d is not None and days > 0:
            ...
            if foreign_5d > 0 and inst_5d > 0:
                ...
                supply_content = f"비상이야! 최근 {days}일간 외놈 {f_abs} 물량과 기관 {i_abs} 물량이 쌍끌이로 쏟아지고 있어. 개미들만 {ind_abs} 온몸으로 물량을 받는 형국이니까 절대 섣불리 물타지 말고 관망하자."
            elif foreign_5d > 0:
                ...
                supply_content = f"최근 {days}일간 외놈 {f_abs} 물량과 기관 {i_abs} 물량이 엇갈리면서 힘겨루기가 팽팽하게 이어지고 있어. 개미 수급도 눈치싸움이 치열하니까 기준 가격만 철저히 지키자."
        else:
        ...
        sections = [
            {
                ...
                "content": f"개미들아! {raw_name} {change_pct:+.2f}% {desc_word}\n현재 주가는 {price_str} 기록 중이야.\n\n{news_lines}\n\n이슈가 전해지면서 세력들의 매매가 요동치고 있으니까 꽉 잡아!",
            ...
            {
                "title": "여기 깨지면 도망쳐라!",
                "content": f"차트 흐름을 보면 20일선 기준 가격이 딱 {ma20_str}이야. 이거 꼭 기억해 놔! 이 가격 깨지면 최근 고점에 물려있던 악성 대기 물량들이 투매로 쏟아질 수 있으니까, 절대 미련 갖지 말고 비중부터 줄이는 게 좋아."
            },
            {
